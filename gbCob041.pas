{****************************************************************************
* $Keywords
* Projeto   : Delphi Boleto
* Unit Name : gbCob041
* Author    : Carlos Augusto
* Criada em : 02/07/2004 10:53:32
* Purpose   : Unit para o Banco 041 - Banrisul
* Obs       : Carlos - 02/07 - Favor testar, eu adaptei do
*             meu Delphi Boleto que é modificado
*****************************************************************************
$Revision:$
$Version:$
$Author:$
...
{*****************************************}
{* Mover para a gbUtil.pas               *}
{* Usado tb pelo banco BRB               *}
{*****************************************}
...
function TgbBanco041.GetNomeBanco: string;
begin
  Result := NomeBanco;
end;

function TgbBanco041.CalcularDigitoNossoNumero(ATitulo: TgbTitulo): string;
var
  ANossoNumero: string;
...
begin
  ...
  ADigitoNossoNumero := CalculaDigitosChaveASBACE(ANossoNumero);

  Result := ADigitoNossoNumero;
end;

function TgbBanco041.GetCampoLivreCodigoBarra(ATitulo: TgbTitulo): string;
var
  ...
  function DV1(ANumero: string): string;
  begin
    lDV1 := Modulo10(ANumero);
    Result := lDV1;
  end;

  function DV2(ANumero: string): string;
  begin
    Result := Modulo11BRB(ANumero, 7);
  end;
...
begin
  ...
  with ATitulo do
    begin
      { Produto + Constante (1) }
      { Produto: 1 - Cobranca nominal
                 2 - Cobranca direta }
      ACodigoCedente := '21' + Formatar(Cedente.ContaBancaria.CodigoAgencia, 3,
        false, '0') +
        Formatar(Cedente.CodigoCedente, 7, false, '0');
      ANossoNumero := Formatar(NossoNumero, 8, false, '0');
      lAux := ACodigoCedente + ANossoNumero + '041';

      Result := lAux + DV1(lAux) + DV2(lAux + lDV1);
    end;
end;

procedure TgbBanco041.FormatarBoleto(ATitulo: TgbTitulo; var AAgenciaCodigoCedente, ANossoNumero, ACarteira, AEspecieDocumento: string);
var
  DV1: string;
  NN: string;
begin
  with ATitulo do
    begin
      ...
      NN := Formatar(NossoNumero, 8, false, '0');
      DV1 := CalculaDigitosChaveASBACE(NN);
      ANossoNumero := NN + '.' + DV1;
...
function TgbBanco041.GerarRemessa(var ACobranca: TgbCobranca; var Remessa:
  TStringList): boolean;
begin
  Result := false;
...
function TgbBanco041.LerRetorno(var ACobranca: TgbCobranca; Retorno: TStringList): boolean;
begin
   TRY
      with ACobranca do
      begin
...
function TgbBanco041.GerarRemessaCNAB240(var ACobranca: TgbCobranca; var Remessa: TStringList): boolean;
var
...
begin
   ...
   begin
      ...
      if Formatar(CodigoBanco,3,false,'0') <> Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Banco.Codigo,3,false,'0')
      then Raise Exception.CreateFmt('O título (Nosso Número: %s) não pertence ao banco %s (%s)',[Titulos[NumeroRegistro].NossoNumero,CodigoBanco,NomeBanco]);
      ...
      Registro := Registro + '0'; {33 a 33 - codigo do convenio no banco}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,19,true,'0'); {34 a 52 - Código do convênio no banco "CONSIDERAR PRIMEIROS 12 DIGITOS"}

      Registro := Registro + Formatar('00',2); {53 a 54- Agencia}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,3,false,'0'); {55 a 57 - Código da agência do cedente}
      Registro := Registro + Formatar('',1); {58 - Dígito do código da agência do cedente}
      ...
      Registro := Registro + Formatar('',1); {72 - Dígito verificador da agência / conta do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.Nome,30,true,' '); {73 a 102 - Nome do cedente}
      Registro := Registro + Formatar('BANRISUL',30,true,' '); {103 a 132 - Nome do banco}
      ...
      Registro := Registro + '040'; {164 a 166 - Número da versão do layout do arquivo}
      ...
      Registro := Registro + '0'; {34 a 34 - codigo do convenio no banco}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,19,true,'0'); {35 a 53 - Código do convênio no banco "CONSIDERAR PRIMEIROS 12 DIGITOS"}
      ...
      Registro := Registro + Modulo11(Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,4,false,'0')+Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.NumeroConta,8,false,'0')); {73 - Dígito verificador da agência / conta}
      ...
      begin
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero + Titulos[NumeroRegistro].DigitoNossoNumero,20,true,'0'); {38 a 57 - Nosso número - identificação do título no banco}
         ...
         Registro := Registro + '00000'; {101 a 105 - Agência cobradora. Deixar zerado. determinará automaticamente pelo CEP do sacado}
      ...
      Registro := Registro + Formatar('',15,false,'0'); {Valor dos títulos em carteiras}
      Registro := Registro + Formatar('',6,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',15,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',6,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',15,false,'0'); {Valor dos títulos em carteiras}
      Registro := Registro + Formatar('',6,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar('',15,false,'0'); {Valor dos títulos em carteiras}
      Registro := Registro + Formatar('',8); {numero do aviso de lancamento}
...
function TgbBanco041.GerarRemessaCNAB400(var ACobranca: TgbCobranca; var Remessa: TStringList): boolean;
var
   ACedenteTipoInscricao, ASacadoTipoInscricao,
   Registro : string;
...
function TgbBanco041.LerRetornoCNAB240(var ACobranca: TgbCobranca; Retorno: TStringList): boolean;
var
   ACodigoBanco,
   ANomeCedente,
   ATipoInscricao,
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  NossoNumero := Copy(Retorno.Strings[NumeroRegistro],38,20);
...
function TgbBanco041.LerRetornoCNAB400(var ACobranca: TgbCobranca; Retorno: TStringList): boolean;
var
   ACodigoBanco,
   ANomeCedente,
